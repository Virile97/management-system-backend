...
function buildGroupRows(groupIds) {
...
function create(data, groupIds, levelId, lighthouseGroupId) {
  return prisma.member.create({
    data: {
      ...
      ...(groupIds ? { groups: { create: buildGroupRows(groupIds) } } : {}),
    },
    include: memberIncludes,
  })
}
...
async function updateById(id, data, groupIds, levelId, lighthouseGroupId) {
  if (groupIds) {
    // Changing the group selection replaces the rows — level/lighthouseGroup
    // live on the member itself now, so this no longer needs to carry them
    // forward onto the new rows.
    ...
    return prisma.member.update({
      ...
      data: {
        ...
        ...(lighthouseGroupId !== undefined ? { lighthouseGroupId } : {}),
        groups: { create: buildGroupRows(groupIds) },
      },
      include: memberIncludes,
    })
  }
